...
# ffmpeg_path="${ffmpeg_path}${ffmpeg_path_postfix_0}"
...
if [[ $ripgrep = y ]] &&
    do_vcs "${ripgrep_path}${ripgrep_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $jo = y ]] &&
    do_vcs "${jo_path}${jo_path_postfix_0}"; then
...
if [[ $jq = y ]] &&
    do_vcs "${jq_path}${jq_path_postfix_0}"; then
...
if [[ $dssim = y ]] &&
    do_vcs "${dssim_path}${dssim_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if { enabled libxml2 || [[ $cyanrip = y ]]; } &&
    do_vcs "${libxml2_path}${libxml2_path_postfix_0}";then
...
if [[ $mplayer = y || $mpv = y ]] ||
    ...
    if do_vcs "${freetype_path}${freetype_path_postfix_1}"; then
    ...
    if enabled_any {lib,}fontconfig &&
        do_vcs "${fontconfig_path}${fontconfig_path_postfix_1}"; then
    ...
    if do_vcs "${harfbuzz_path}${harfbuzz_path_postfix_0}"; then
    ...
    if do_vcs "${fribidi_path}${fribidi_path_postfix_0}"; then
    ...
    if do_vcs "${libass_path}${libass_path_postfix_0}"; then
        do_autoreconf
...
if [[ $curl = libressl ]] || { [[ $ffmpeg != no ]] && enabled libtls; }; then
    ...
    if do_vcs "${libressl_path}${libressl_path_postfix_1}" libressl; then
...
if [[ $mediainfo = y || $bmx = y || $curl != n || $cyanrip = y ]] &&
    do_vcs "${curl_path}${curl_path_postfix_1}"; then
...
if { { [[ $ffmpeg != no || $standalone = y ]] && enabled libtesseract; } ||
    ...
    if do_vcs "${freeglut_path}${freeglut_path_postfix_0}" freeglut; then
    ...
    if do_vcs "${libtiff_path}${libtiff_path_postfix_0}"; then
...
if [[ $ffmpeg != no && -f $opencldll ]] && enabled opencl; then
    ...
    _check=(CL/cl.h)
    if do_vcs "${OpenCL_Headers_path}${OpenCL_Headers_path_postfix_0}"; then
...
if [[ $ffmpeg != no || $standalone = y ]] && enabled libtesseract; then
    ...
    if do_vcs "${leptonica_path}${leptonica_path_postfix_1}"; then
    ...
    if do_vcs "${tesseract_path}${tesseract_path_postfix_0}"; then
...
if { { [[ $ffmpeg != no ]] && enabled librubberband; } ||
    ...
    do_vcs "${rubberband_path}${rubberband_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libzimg &&
    do_vcs "${zimg_path}${zimg_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libilbc &&
    do_vcs "${libilbc_path}${libilbc_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $standalone = y ]] && enabled libspeex &&
    ...
    do_vcs "${speex_path}${speex_path_postfix_0}"; then
...
if [[ $flac = y ]] && do_vcs "${flac_path}${flac_path_postfix_0}"; then
...
if [[ $standalone = y && $faac = y ]] &&
    do_vcs "${faac_path}${faac_path_postfix_0}"; then
...
if [[ $exhale = y ]] &&
    do_vcs "${exhale_path}${exhale_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
    _notrequired=true
...
if [[ $standalone = y ]] && enabled libvorbis &&
    do_vcs "${vorbis_tools_path}${vorbis_tools_path_postfix_0}"; then
...
if enabled libopus && do_vcs "${opus_path}${opus_path_postfix_0}"; then
...
if [[ $standalone = y ]] && enabled libopus; then
    ...
    if do_vcs "${opusfile_path}${opusfile_path_postfix_0}"; then
        do_uninstall "${_check[@]}"
    ...
    _deps=(opus.pc)
    if do_vcs "${libopusenc_path}${libopusenc_path_postfix_0}"; then
        do_uninstall "${_check[@]}"
        do_autogen
        do_separate_confmakeinstall --disable-{examples,doc}
        do_checkIfExist
    ...
    if do_vcs "${opus_tools_path}${opus_tools_path_postfix_0}"; then
...
if [[ $ffmpeg != no ]] && enabled libsoxr &&
    do_vcs "${libsoxr_path}${libsoxr_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libcodec2; then
    if do_vcs "${codec2_path}${codec2_path_postfix_0}"; then
...
if [[ $sox = y ]] && do_vcs "${sndfile_path}${sndfile_path_postfix_0}" sndfile; then
...
if [[ $ffmpeg != no ]] && enabled libopenmpt &&
    do_vcs "${openmpt_path}${openmpt_path_postfix_1}"; then
...
if [[ $ffmpeg != no ]] && enabled libmysofa &&
    do_vcs "${libmysofa_path}${libmysofa_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if enabled libflite && do_vcs "${flite_path}${flite_path_postfix_0}"; then
...
if { { [[ $ffmpeg != no ]] &&
    ...
    do_vcs "${openal_soft_path}${openal_soft_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if { [[ $rtmpdump = y ]] ||
    ...
    do_vcs "${librtmp_path}${librtmp_path_postfix_0}" librtmp; then
...
if [[ $ffmpeg != no ]] && enabled libvmaf &&
    do_vcs "${vmaf_path}${vmaf_path_postfix_0}"; then
...
if { [[ $dav1d = y ]] || [[ $libavif = y ]] || { [[ $ffmpeg != no ]] && enabled libdav1d; }; } &&
    do_vcs "${dav1d_path}${dav1d_path_postfix_0}"; then
...
if { enabled librav1e || [[ $libavif = y ]]; } &&
    do_vcs "${cargo_c_path}${cargo_c_path_postfix_0}"; then
...
if { [[ $rav1e = y ]] || [[ $libavif = y ]] || enabled librav1e; } &&
    do_vcs "${rav1e_path}${rav1e_path_postfix_0}"; then
...
if [[ $libavif = y ]] && {
    ...
    do_vcs "${libavif_path}${libavif_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $jpegxl = y ]] && do_vcs "${jpeg_xl_path}${jpeg_xl_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if { [[ $other265 = y ]] || { [[ $ffmpeg != no ]] && enabled libkvazaar; }; } &&
    do_vcs "${kvazaar_path}${kvazaar_path_postfix_0}"; then
...
if { { [[ $ffmpeg != no ]] &&
    ...
    mpv_enabled sdl2; } &&
    do_vcs "${SDL_path}${SDL_path_postfix_0}"; then
...
if { [[ $mplayer = y ]] || mpv_enabled dvdnav; } &&
    do_vcs "${dvdread_path}${dvdread_path_postfix_0}" dvdread; then
    do_autoreconf
...
_deps=(libdvdread.a)
if { [[ $mplayer = y ]] || mpv_enabled dvdnav; } &&
    do_vcs "${dvdnav_path}${dvdnav_path_postfix_0}" dvdnav; then
    do_autoreconf
...
if { [[ $ffmpeg != no ]] && enabled libbluray; } || ! mpv_disabled libbluray; then
    ...
    if do_vcs "${libaacs_path}${libaacs_path_postfix_0}"; then
    ...
    if do_vcs "${libbdplus_path}${libbdplus_path_postfix_0}"; then
...
if { { [[ $ffmpeg != no ]] && enabled libbluray; } || ! mpv_disabled libbluray; } &&
    do_vcs "${libbluray_path}${libbluray_path_postfix_0}"; then
...
if [[ $ffmpeg != no ]] && enabled libxavs && do_pkgConfig "xavs = 0.1." "0.1" &&
    do_vcs "${xavs_path}${xavs_path_postfix_0}"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "${xavs2_path}${xavs2_path_postfix_0}"; then
    cd_safe build/linux
    [[ -f config.mak ]] && log "distclean" make distclean
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "${davs2_path}${davs2_path_postfix_0}"; then
    cd_safe build/linux
    [[ -f config.mak ]] && log "distclean" make distclean
    do_uninstall all "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libuavs3d &&
    do_vcs "${uavs3d_path}${uavs3d_path_postfix_0}"; then
    do_cmakeinstall
...
if [[ $mediainfo = y ]]; then
    [[ $curl = openssl ]] && hide_libressl
    _check=(libzen.{a,pc})
    if do_vcs "${libzen_path}${libzen_path_postfix_0}" libzen; then
    ...
    if do_vcs "${libmediainfo_path}${libmediainfo_path_postfix_0}" libmediainfo; then
    ...
    if do_vcs "${mediainfo_path}${mediainfo_path_postfix_0}" mediainfo; then
...
if [[ $ffmpeg != no ]] && enabled libvidstab &&
    do_vcs "${vidstab_path}${vidstab_path_postfix_0}" vidstab; then
...
if [[ $ffmpeg != no ]] && enabled_any frei0r ladspa; then
    ...
    if do_vcs "${dlfcn_win32_path}${dlfcn_win32_path_postfix_0}"; then
        do_uninstall "${_check[@]}"
        do_cmakeinstall
        do_checkIfExist
    fi
    ...
    _check=(frei0r.{h,pc})
    if do_vcs "${frei0r_path}${frei0r_path_postfix_0}"; then
...
if [[ $ffmpeg != no ]] && enabled decklink &&
    do_vcs "${decklink_headers_path}${decklink_headers_path_postfix_0}"; then
...
if [[ $ffmpeg != no ]] && enabled libmfx &&
    do_vcs "${libmfx_path}${libmfx_path_postfix_0}" libmfx; then
    do_autoreconf
...
if [[ $ffmpeg != no ]] && { enabled amf || ! disabled_any autodetect amf; } &&
    do_vcs "${AMF_path}${AMF_path_postfix_0}"; then
...
if [[ $mp4box = y ]] && do_vcs "${gpac_path}${gpac_path_postfix_0}"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "${SVT_HEVC_path}${SVT_HEVC_path_postfix_0}"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "${SVT_AV1_path}${SVT_AV1_path_postfix_0}"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "${SVT_VP9_path}${SVT_VP9_path_postfix_0}"; then
...
if [[ $xvc == y ]] &&
    do_vcs "${xvc_path}${xvc_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $x264 != no ]]; then
    ...
    if do_vcs "${x264_path}${x264_path_postfix_0}" ||
        ...
        if [[ $standalone = y && $x264 =~ (full|fullv) ]]; then
            ...
            do_vcs "${ffmpeg_path}${ffmpeg_path_suffix}" ffmpeg
            ...
            if do_vcs "${ffms2_path}${ffms2_path_postfix_0}"; then
                do_uninstall "${_check[@]}"
        ...
        if [[ $standalone = y ]]; then
            ...
            if do_vcs "${liblsmash_path}${liblsmash_path_postfix_0}" liblsmash; then
                [[ -f config.mak ]] && log "distclean" make distclean
                do_uninstall "${_check[@]}"
                create_build_dir
...
if [[ ! $x265 = n ]] && do_vcs "${x265_git_path}${x265_git_path_postfix_0}"; then
...
if enabled libxvid && [[ $standalone = y ]] &&
    do_vcs "${xvid_path}${xvid_path_postfix_0}"; then
...
if [[ $ffmpeg != no ]] && { enabled ffnvcodec ||
    ...
    do_vcs "${ffnvcodec_path}${ffnvcodec_path_postfix_0}" ffnvcodec; then
    do_makeinstall PREFIX="$LOCALDESTDIR"
    do_checkIfExist
fi
...
if enabled libsrt && do_vcs "${srt_path}${srt_path_postfix_0}"; then
    do_pacman_install openssl
    hide_libressl
...
if enabled librist && do_vcs "${librist_path}${librist_path_postfix_0}"; then
...
if [[ $ffmpeg != no ]] && enabled liblensfun &&
    ...
    do_vcs "${lensfun_path}${lensfun_path_postfix_0}"; then
...
if [[ $bits = 64bit && $vvc = y ]] &&
    do_vcs "${vvc_path}${vvc_path_postfix_0}" vvc; then
...
if [[ $ffmpeg != no ]] && enabled avisynth &&
    do_vcs "${AviSynthPlus_path}${AviSynthPlus_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if { { [[ $ffmpeg != no ]] && enabled vulkan; } || ! mpv_disabled vulkan; } &&
    do_vcs "${vulkan_path}${vulkan_path_postfix_0}" vulkan-loader; then
...
if { { [[ $mpv != n ]]  && ! mpv_disabled libplacebo; } ||
    ...
    do_vcs "${glslang_path}${glslang_path_postfix_0}"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]]; then
    ...
    if do_vcs "${ffmpeg_path}${ffmpeg_path_suffix}" ffmpeg; then
...
if [[ $mplayer = y ]] && check_mplayer_updates; then
    ...
    if [[ ! -d ffmpeg ]] &&
        ...
        git clone -q "${ffmpeg_path}${ffmpeg_path_suffix}" ffmpeg; } &&
        ! git clone "${ffmpeg_path}${ffmpeg_path_suffix}" ffmpeg; then
        rm -rf ffmpeg
        printf '%s\n' \
...
if [[ $mpv != n ]] && pc_exists libavcodec libavformat libswscale libavfilter; then
    if ! mpv_disabled lua && opt_exists MPV_OPTS "--lua=5.1"; then
        ...
        do_vcs "${luajit_path}${luajit_path_postfix_0}" luajit; then
    ...
    if ! mpv_disabled javascript &&
        do_vcs "${mujs_path}${mujs_path_postfix_0}"; then
    ...
    if mpv_enabled mruby && do_vcs "${mruby_path}${mruby_path_postfix_0}"; then
    ...
    if ! mpv_disabled shaderc &&
        do_vcs "${shaderc_path}${shaderc_path_postfix_0}"; then
        ...
        add_third_party "${glslang_path}${glslang_path_postfix_0}"
        add_third_party "${spirv_path}${spirv_path_postfix_0}" spirv-tools
        add_third_party "${spirv_SPIRV_Headers_path}${spirv_SPIRV_Headers_path_postfix_0}" spirv-headers
        add_third_party "${spirv_SPIRV_Cross_path}${spirv_SPIRV_Cross_path_postfix_0}" spirv-cross
    ...
    if ! mpv_disabled spirv-cross &&
        do_vcs "${spirv_SPIRV_Cross_path}${spirv_SPIRV_Cross_path_postfix_0}"; then
    ...
    if ! mpv_disabled libplacebo &&
        do_vcs "${libplacebo_path}${libplacebo_path_postfix_0}"; then
    ...
    if do_vcs "${mpv_path}${mpv_path_postfix_0}"; then
        hide_conflicting_libs
        create_ab_pkgconfig
...
if [[ $bmx = y ]]; then
    ...
    if do_vcs "${libMXF_path}${libMXF_path_postfix_0}" libMXF-1.0; then
        do_autogen
    ...
    _deps=(libMXF-1.0.a)
    if do_vcs "${libMXF_libmxfpp_path}${libMXF_libmxfpp_path_postfix_0}" libMXF++-1.0; then
        do_autogen
        do_uninstall include/libMXF++-1.0 "${_check[@]}"
        do_separate_confmakeinstall video --disable-examples
        do_checkIfExist
    ...
    if do_vcs "${bmx_path}${bmx_path_postfix_0}"; then
        do_autogen
...
if [[ $cyanrip = y ]]; then
    ...
    if do_vcs "${neon_path}${neon_path_postfix_0}"; then
    ...
    if do_vcs "${libmusicbrainz_path}${libmusicbrainz_path_postfix_0}"; then
    ...
    if do_vcs "${cyanrip_path}${cyanrip_path_postfix_0}"; then
        old_PKG_CONFIG_PATH=$PKG_CONFIG_PATH
        _check=("$LOCALDESTDIR"/opt/cyanffmpeg/lib/pkgconfig/libav{codec,format}.pc)
        if flavor=cyan do_vcs "${ffmpeg_path}${ffmpeg_path_suffix}" ffmpeg; then
...
if [[ $vlc == y ]]; then
    ...
    if do_vcs "${fxc2_path}${fxc2_path_postfix_0}"; then
        do_uninstall "${_check[@]}"
...
EOF
    ...
    if do_vcs "${libspatialaudio_path}${libspatialaudio_path_postfix_0}"; then
    ...
    if do_vcs "${libshout_path}${libshout_path_postfix_0}" libshout; then
        do_uninstall "${_check[@]}"
    ...
    if do_vcs "${protobuf_path}${protobuf_path_postfix_0}"; then
    ...
    if do_vcs "${pixman_path}${pixman_path_postfix_0}"; then
    ...
    if do_vcs "${medialibrary_path}${medialibrary_path_postfix_0}"; then
    ...
    if do_vcs "${libthai_path}${libthai_path_postfix_0}"; then
    ...
    if do_vcs "${libebml_path}${libebml_path_postfix_0}"; then
    ...
    if do_vcs "${libmatroska_path}${libmatroska_path_postfix_0}"; then
    ...
    if do_vcs "${vlc_path}${vlc_path_postfix_0}"; then
...
if [[ $ffmbc = y ]] && do_vcs "${FFmbc_path}${FFmbc_path_postfix_1}"; then # no other branch
    _notrequired=true
    create_build_dir
...
if [[ $redshift = y ]] && do_vcs "${redshift_path}${redshift_path_postfix_0}"; then
